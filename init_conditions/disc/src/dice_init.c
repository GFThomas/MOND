...
int allocate_variable_arrays(galaxy *gal) {
    ...
    // Allocate mass for all the particles. -WHATZAAAA
    if (!(gal->w = calloc(gal->ntot_part_pot,sizeof(unsigned long int)))) {
        fprintf(stderr,"[Error] Unable to allocate particle weights.\n");
        return -1;
    }
...
int reallocate_variable_arrays(galaxy *gal, unsigned long int npart) {
    ...
    // Allocate w coordinates for all particles // WHATZAA
    gal->w = (double *) realloc(gal->w,npart*sizeof(double));
    if (gal->w == NULL) {
        fprintf(stderr,"[Error] Unable to allocate particle weights.\n");
        return -1;
    }
...
int create_galaxy(galaxy *gal, char *fname, int info) {
    ...
    //Define MOND variables for later use (WHATZAA)
    //double mond_a0;
    //int mond_forces;
    ...
    if (info != 0) {
        ...
        // printf("/////\t\t- Gas fraction = %10.5lf \n", gal->gfrac);
        // printf("/////\t\t- Gas Temperature = %12.3lf \n", gal->gtemp);
    ...
    printf("/////\t--------------------------------------------------\n");
    //lower_resolution(gal); //We have disabled this
    // Release memory
...
int set_galaxy_coords(galaxy *gal) {
    ...
    for(i = 0; i<AllVars.MaxCompNumber; i++) {
        if(gal->comp_npart[i]>0) {
	    printf("////// max Component %2d:",AllVars.MaxCompNumber);
	    if(gal->comp_npart[i]>1) {
            ...
            //printf("\n Fixing particle masses \n");
            puts("Fixing particle masses.");
 	    fix_masses(gal,i);
        }
    }
...
int set_galaxy_velocity(galaxy *gal) {
    ...
    FILE *fp1, *fp2;
    char outfilename[30], buffer[MAXLEN_FILENAME], ext[20]; //%IB added outfilename.
    ...
    double u_min, maxrad, maxrad_gas, k_stream, kmax_stream, surface_density, g_N_r, g_N_z; //Variables after k_max_stream added by IB.
    double radius, interval, save1, save2, save3, save4, save5;
    double sigma_sq;//, sigma_sq_gradient, sigma_sq_last, sigma_sq_second_last, interval_inv, h_stars; //Added by IB.
    double h, h_stars, h_1, h_2, Error, Error_1, Error_2, Tolerance, Gradient_inv, Sigma_stars, Sigma_gas, tanh_1_inv;
    double Min_Q, Min_Q_location, u, u_1, u_2, force, dforcedr, kappa_sqrd, g_N_total, Half_K_0, G_Sigma_eff;
    //Debug: //double Sigma_total;
    ...
    strcat(outfilename, "_rotation_curve.txt");
    ...
    for(j = 0; j<AllVars.MaxCompNumber; j++) {
        ...
        if(gal->comp_sigmar_model[j]>0) {
            ...
            gal->comp_sigmar_scale[j] = pow(gal->comp_sigmar[j],2) / surface_density_func_IB(gal,gal->r_cyl[i]); //surface_density_func(gal,gal->r_cyl[i],gal->theta_cyl[i],1,j);
            gal->z[gal->index[tid]] = save1;
            gal->theta_cyl[gal->index[tid]] = save2;
        ...
        if(gal->comp_sigmaz_model[j]>0) {
            ...
            gal->comp_sigmaz_scale[j] = pow(gal->comp_sigmaz[j],2) / surface_density_func_IB(gal,gal->r_cyl[i]); //surface_density_func(gal,gal->r_cyl[i],gal->theta_cyl[i],1,j);
            gal->z[gal->index[tid]] = save1;
            gal->theta_cyl[gal->index[tid]] = save2;
            gal->r_cyl[gal->index[tid]] = save3;
            gal->x[gal->index[tid]] = save4;
...
#pragma omp parallel for private(v_c, v_r, v_theta, v_z, v2a_r, v2a_z, v2a_theta, v2_theta, va_theta, sigma2_theta, vel_x, vel_y, vel_z, Q, save) shared(j,gal) reduction(+:nrejected,nrejected_failed)
            ...
            if(va_theta>v_stream_max && gal->comp_type[j]>0) v_stream_max = va_theta; 
            ...
            // Case of a spherical pressure supported disitribution
                ...
                if(v_theta>v_stream_max) v_stream_max = v_theta; 
            ...
            // ------ Cylindrical coordinates ------
                ...
                // No Jeans equations
                    case 0:
                                    ...
                                    if(gal->comp_sigmar_model[j]==0) {
                                        surface_density = surface_density_func_IB(gal,gal->r_cyl[i]); //surface_density_func(gal,gal->r_cyl[i],gal->theta_cyl[i],1,j);
                                        g_N_r = galaxy_rforce_func_Newton(gal,gal->r_cyl[i],0.0);
                                        g_N_z = 2.0*pi*G*surface_density;
                                        v2a_r = pi*G*mond_nu_function(sqrt(g_N_r*g_N_r + g_N_z*g_N_z))*surface_density*gal->comp_scale_length[j]*gal->comp_flatz[j];
                                    // Dispersion proportional to surface density
                                    } else if (gal->comp_sigmar_model[j]>0) {
                                        save = gal->comp_model[j];
                                        gal->comp_model[j] = gal->comp_sigmar_model[j];
                                        v2a_r = gal->comp_sigmar_scale[j]*surface_density_func_IB(gal,gal->r_cyl[i]); //surface_density_func(gal,gal->r_cyl[i],gal->theta_cyl[i],1,j);
                                        gal->comp_model[j] = save;
                                    }
                                    // Dispersion from Isothermal sheet
                                    if(gal->comp_sigmaz_model[j]==0) {
                                        ...
                                        g_N_r = galaxy_rforce_func_Newton(gal,gal->r_cyl[i],0.0);
                                        g_N_z = 2.0*pi*G*surface_density;
                                        v2a_z = pi*G*mond_nu_function(sqrt(g_N_r*g_N_r + g_N_z*g_N_z))*surface_density*gal->comp_scale_length[j]*gal->comp_flatz[j];
                                    // Dispersion proportional to surface density
                                    } else if (gal->comp_sigmar_model[j]>0) {
                                        save = gal->comp_model[j];
                                        gal->comp_model[j] = gal->comp_sigmaz_model[j];
                                        v2a_z = gal->comp_sigmaz_scale[j]*surface_density_func_IB(gal,gal->r_cyl[i]); //surface_density_func(gal,gal->r_cyl[i],gal->theta_cyl[i],1,j);
                                        gal->comp_model[j] = save;
                                    }
                                    v2a_theta = v2a_theta_2D_func(gal,fabs(gal->r_cyl[i]),v2a_r,v_c,j);
                    break;
                ...
                if(v2a_r > gal->r_cyl[i]*galaxy_rforce_func(gal, gal->r_cyl[i], gal->z[i])) v2a_r = sigma2_theta;
                //if(v2a_r < 0.0) v2a_r = 0.0; //There is already a safety on v2a_theta.
                            ...
                            //puts("Allocating random velocity here."); //This actually works, don't try it.
                            // Escape velocity
                ...
                vmax = fabs(gal->comp_vmax_esc[j])*sqrt(2*fabs(galaxy_total_potential(gal,gal->x[i],gal->y[i],gal->z[i],1,0)));
                            ...
                            if(gal->comp_vmax_circ[j]>0.) {
                                vmax = min(gal->comp_vmax_circ[j]*v_cmax,vmax); //min changed to max by IB.
                            }
...
int rotate_galaxy(galaxy *gal, int index) {
    ...
    puts("IB attempted to suppress rotation of galaxy.");
    return 0;
...
int rotate_component(galaxy *gal, double alpha, double delta, int component) {
    ...
    // Alpha is the spin angle
    // Delta is the inclination of the disk compare to the XY plane
    puts("IB attempted to suppress rotation of galaxy components.");
    return 0;
    alpha = alpha*pi/180.;
    delta = delta*pi/180.;
    ...
    puts("Galaxy component rotated anyway.");
    return 0;
}
...
int rotate_all(galaxy *gal, double alpha, double delta, int order) {
    ...
    // Alpha is the spin angle
    // Delta is the inclination of the disk compare to the XY plane
    puts("IB attempted to suppress rotation of galaxy particles.");
    return 0;
    alpha = alpha*pi/180.;
    delta = delta*pi/180.;
    switch(order) {
    ...
    puts("Galaxy particles rotated anyway.");
    return 0;
}
...
int rotate_stream(galaxy *st, int index) {
    ...
    // Alpha is the spin angle
    // Delta is the inclination of the disk compare to the XY plane
    puts("IB attempted to suppress rotation of stream.");
    return 0;
...
int copy_galaxy(galaxy *gal_1, galaxy *gal_2, int info) {
    ...
    for (j = 0; j < AllVars.MaxCompNumber; ++j) {
	    ...
	    gal_2->comp_stream_scale[j] = gal_1->comp_stream_scale[j];
        }
    }
    ...
    for (j = 0; j < AllVars.MaxCompNumber; ++j) {
		...
		gal_2->w[k] = gal_1->w[i]; //%Added by IB.
...
int add_galaxy_to_system(galaxy *gal_1, galaxy *gal_2) {
    ...
    for(k = 0; k<AllVars.MaxCompNumber; k++) {
		...
		gal_2->w[a] = gal_1->w[i]; //%Added by IB.
