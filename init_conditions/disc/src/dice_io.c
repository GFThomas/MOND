...
int write_gadget2_ics(galaxy *gal, char *fname) {
    ...
    //unsigned long int i, j, k; //i, j and k should be moved to next line.
    int i, j, k, t,n,off,pc,pc_new,pc_sph, k_print; //k_print added by IB.
    int files = 1;
    char buf[200];
    #define SKIP2 fwrite(&dummy, sizeof(dummy), 1, fp1);
    ...
    char ptype_str[2];
    char k_str[2];
    double x_pos, y_pos, z_pos, x_vel, y_vel, z_vel, mass, Gconv;
    double age, metal, int_energy, rho, r, rc, weight, Sum_z_values[50]; //Roy added weight
    int gastag;
    int ascii_out_hack = 1, nan_fixer = 1;
    int nans, fixed_nans;
    ...
    if (nan_fixer){
        nans = 0;
        fixed_nans = 0;
        printf("******** CHECKING FOR NAN PROBLEMS *************\n");
        for (ptype=0; ptype<10; ptype++) {
            for (k=0;k<AllVars.MaxCompNumber;k++) {
		if(gal->comp_type[k]==ptype&&gal->comp_npart[k]>0&&gal->comp_delete[k]==0) {
                    for (i = gal->comp_start_part[k]; i < gal->comp_start_part[k] + gal->comp_npart[k]; ++i) {
                        //weirdly if a float is NaN it always gives false with
                        //any logical comparison, even with itself
                        if (gal->vel_x[i] != gal->vel_x[i]) {
                            nans = nans + 1;
                            printf("%d, VX: %f \n",i,gal->vel_x[i]);
                            printf("%d, VY: %f \n",i,gal->vel_y[i]);
                            printf("%d, VZ: %f \n",i,gal->vel_z[i]);
                            printf("%d, U: %f \n",i,gal->u[i]);
                            rc = sqrt(pow(gal->x[i],2) + pow(gal->y[i],2) + pow(gal->z[i],2));
                            for (j = gal->comp_start_part[k]; j < gal->comp_start_part[k] + gal->comp_npart[k]; ++j) {
                                if (j == i) {
                                    continue;
                                }
                                if (gal->vel_x[j] != gal->vel_x[j]) {
                                    continue;
                                }
                                r = sqrt(pow(gal->x[j],2) + pow(gal->y[j],2) + pow(gal->z[j],2));
                                if ( fabs(r - rc) < 0.1 ) {
                                    gal->vel_x[i] = gal->vel_x[j];
                                    gal->vel_y[i] = gal->vel_y[j];
                                    gal->vel_z[i] = gal->vel_z[j];
                                    gal->u[i] = gal->u[j];
                                    fixed_nans = fixed_nans + 1;
                                    break;
                                }
                            }
                        }
    ...
    //Graeme's Addition
    if (ascii_out_hack){
        printf("******** ASCII FILE OF PARTICLES WILL BE WRITTEN *************\n");
        for (ptype=0; ptype<10; ptype++) {
            k = 0;
            if(gal->comp_type[k]==ptype&&gal->comp_npart[k]>0&&gal->comp_delete[k]==0) {
                //Change galaxy names here and in DICE_init set_galaxy_velocity function.
            	...
            	strcat(outfilename, "_k");
            	strcat(outfilename, k_str);
            	strcat(outfilename, ".txt");
            	FILE *f = fopen(outfilename, "w");
            	printf("Outfilename: %s \n", outfilename);
                puts("WARNING: output file in Solar masses, although DICE mass unit = 1e10 M_Sun.");
                ...
                for (k=0;k<AllVars.MaxCompNumber;k++) {
		          for (i = gal->comp_start_part[k]; i < gal->comp_start_part[k] + gal->comp_npart[k]; ++i) {
                        ...
                        z_vel = gal->vel_z[i];
                        weight = gal->w[i];
                        mass = AllVars.UnitMass/solarmass*gal->mass[i];
                        int_energy = gal->u[i];
                        rho = gal->rho[i];
                        metal = gal->metal[i];
                        age = gal->age[i];
                        //fprintf(f, "%d,%f,%f,%f,%f,%f,%f,%.8e,%.8e,%f,%f,%f\n",gastag, x_pos,y_pos,z_pos,x_vel,y_vel,z_vel,weight,mass,metal,age,int_energy);// Roy adding weight to this output, note that it is the 8th entry (which is after v_z).
                        fprintf(f, "%f,%f,%f,%f,%f,%f,%.8e\n", x_pos,y_pos,z_pos,x_vel,y_vel,z_vel,mass);
                    }
                }
                fclose(f);
    ...
    puts("IB attempted to stop production of dice_highz.g2 file.");
    return 0;
    ...
    free(P);
    //free(Ids);
    puts("dice_highz.g2 file produced anyway.");
    return 0;
}
...
void write_galaxy_rz_quantities(galaxy *gal, double rmax, char *fname, double interval) {
    ...
    double k_stream, kmax_stream, surface_density, g_N_r, g_N_z, Q_min, Q_max; //Variables after k_max_stream added by IB.
    char filename[200];
    FILE *fp1;
    ...
    for (i = -1; i < (int)((rmax-rmin)/loginterval); ++i) {
		...
		// No Jeans equations
	        case 0:
                    ...
                    if(gal->comp_sigmar_model[k]==0) {
                        surface_density = surface_density_func_IB(gal,gal->r_cyl[ii]); //surface_density_func(gal,gal->r_cyl[ii],gal->theta_cyl[ii],1,k)
                        g_N_r = galaxy_rforce_func_Newton(gal,gal->r_cyl[ii],0.0);
                        g_N_z = 2.0*pi*G*surface_density;
                        v2ar = pi*G*surface_density*mond_nu_function(sqrt(g_N_r*g_N_r + g_N_z*g_N_z))*gal->comp_scale_length[k]*gal->comp_flatz[k];
                    // Dispersion proportional to surface density
                        ...
                        v2ar = gal->comp_sigmar_scale[k]*surface_density_func_IB(gal,gal->r_cyl[ii]); //surface_density_func(gal,gal->r_cyl[ii],gal->theta_cyl[ii],1,k);
                    ...
                    if(gal->comp_sigmaz_model[k]==0) {
                        ...
                        g_N_r = galaxy_rforce_func_Newton(gal,gal->r_cyl[ii],0.0);
                        g_N_z = 2.0*pi*G*surface_density;
                        v2az = pi*G*surface_density*mond_nu_function(sqrt(g_N_r*g_N_r + g_N_z*g_N_z))*gal->comp_scale_length[k]*gal->comp_flatz[k]; //IB multiplied G by nu on this line.
                    // Dispersion proportional to surface density
                    } else if (gal->comp_sigmar_model[k]>0) {
                        save = gal->comp_model[k];
                        gal->comp_model[k] = gal->comp_sigmaz_model[k];
                        v2az = gal->comp_sigmaz_scale[k]*surface_density_func_IB(gal,gal->r_cyl[ii]);//surface_density_func(gal,gal->r_cyl[ii],gal->theta_cyl[ii],1,k);
                        gal->comp_model[k] = save;
                    }
                    v2at = v2a_theta_2D_func(gal,fabs(gal->r_cyl[ii]),v2ar,vc,k);
		    break;
	    ...
	    switch(gal->comp_stream_method[k]) {
            ...
            printf("Minimum attained value of Q = %8.4lf and maximum = %8.4lf]\n",Q_min,Q_max);
