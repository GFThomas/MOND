...
#define pi                      3.14159265358979    // pi = 4.0*atan(1.0)
...
#define mond_a0                 3702.8 //mond_a0_cgs(=1.2e-8)*unit_length/(unit_velocity*unit_velocity) //mond_a0_cgs/(unit_velocity*unit_velocity)//DO NOT DO: mond_a0_cgs*unit_length/(unit_velocity*unit_velocity)
#define mond_forces             1 //FOR LONG EXPRESSIONS, ALWAYS PUT BRACKETS AROUND IT AS EXPRESSION ON RIGHT LITERALLY SUBSTITUTED FOR THAT ON LEFT.
#define alpha_IB                3.0 //Used to adjust radial mass (and thus probability) distribution of particles.
#define r_outer_limit_comp1     101. //101.0
#define r_outer_limit_comp2     43. //43.0
#define r_outer_limit_comp3     43.
#define r_outer_limit_comp4     43.
#define r_outer_limit_comp5     43.
#define r_outer_limit_comp6     43.
#define r_outer_limit_comp7     43.
#define r_outer_limit_comp8     43.
...
typedef struct {
    ...
    double              *r_sph;
    double              *phi_sph;
    double		*w; //mass of particles r*exp(-r/r_d). -ADDED (Not a coordinate vector)
    // Velocities vectors
    double              *vel_x;
    double              *vel_y;
    double              *vel_z;
...
typedef struct {
    ...
    double              *phi_sph;
    // Mass vector
    double		*w; //mass of particles r*exp(-r/r_d). -ADDED
    double              *mass;
    // Density vector
    double              *rho;
    // Internal energy vector
...
void fix_masses(galaxy *, int);
double density_functions_stream_pool(stream *, double, double, double, int, int);
double density_functions_stream_pool_roy(stream *, double, double, double, int, int);

double surface_density_func(galaxy *, double, double, int, int);
double surface_density_func_IB(galaxy *, double); //Function added by IB.
double surface_density_func_IB_stars(galaxy *, double); //Function added by IB.
double enclosed_gNz_func(galaxy *, double,  double, double); //Function added by IB.
...
double mond_nu_function(double f); //%Added by IB.
double mond_K_0_function(double f); //%Added by IB.
...
double galaxy_zforce_func(galaxy *, double, double);
double galaxy_rforce_func(galaxy *, double, double);
double galaxy_rforce_func_Newton(galaxy *, double, double); //%Added by IB.
