...
double density_functions_pool(galaxy *gal, double radius, double theta, double z, int cut, int model, int component) {
    ...
    switch(model) {
        ...
        case 2:
            ...
            if(strcmp(gal->comp_profile_name[component],"")==0)
                strcpy(gal->comp_profile_name[component],"      Miyamoto-Nagai      ");
            w = sqrt(z*z+hz*hz);
...
double density_functions_pool_roy(galaxy *gal, double radius, double theta, double z, int cut, int model, int component) {
    double h, hx, hy, hz, h_cut, hx_cut, hy_cut, hz_cut, hx_cut_in, hy_cut_in, hz_cut_in, r_outer_limit; //r_outer_limit added.
    double density, w, k, l, m, n, o, r, s, alpha, beta, smooth_factor1, smooth_factor2, sigma1, sigma2, r_sph, r_flat;
    double x, y, flatx, flaty, flatz, x2, y2, z2;
    double theta_shift, theta_out, A, B, CEDF, tanh_func;
    double z_shift, rcore, xvar, yvar, zvar, usex, usey, usez;

    //Remember this is for a specific template size.
    if(component == 0) {
        r_outer_limit = r_outer_limit_comp1;
    } else if (component == 1) {
        r_outer_limit = r_outer_limit_comp2;
    } else if (component == 2) {
        r_outer_limit = r_outer_limit_comp3;
    } else if (component == 3) {
        r_outer_limit = r_outer_limit_comp4;
    } else if (component == 4) {
        r_outer_limit = r_outer_limit_comp5;
    } else if (component == 5) {
        r_outer_limit = r_outer_limit_comp6;
    } else if (component == 6) {
        r_outer_limit = r_outer_limit_comp7;
    } else if (component == 7) {
        r_outer_limit = r_outer_limit_comp8;
    } else if (component == 8) {
        r_outer_limit = r_outer_limit_comp9;
    } else {
        r_outer_limit = r_outer_limit_comp10;
    }
    ...
    k = sqrt(pow(z/hz,2.0));
    r_flat = sqrt(pow(x,2.0)+pow(y,2.0));
    l = sqrt(pow(x/hx,2.0)+pow(y/hy,2.0));
    m = sqrt(pow(x/hx,2.0)+pow(y/hy,2.0)+pow(z/hz,2.0));
    n = sqrt(pow(usex*x/hx_cut,2.0)+pow(usey*y/hy_cut,2.0)+pow(usez*z/hz_cut,2.0));
    o = sqrt(pow(x/hx_cut,2.0)+pow(y/hy_cut,2.0));
    ...
    switch(model) {
        case 1:
            ...
            if(strcmp(gal->comp_profile_name[component],"")==0)
                strcpy(gal->comp_profile_name[component]," Exponential disk/sech z ");
	    if (r_flat > r_outer_limit) // || (r_flat < 0.0001) //Need bracket round whole if condition.
		density = 0;
	    else
	    	density = gal->comp_scale_dens[component]/(pow(cosh(z/hz),2));
    	    	//density = gal->comp_scale_dens[component]*exp(-l)/(pow(cosh(z/hz),2));
            break;
	case 3:
            // Exponential disk + sech z-profile
...
void mcmc_metropolis_hasting_ntry(galaxy *gal, int component, int density_model) { //alpha_IB removed.
    unsigned long int start_part, npart;
    int i, k, selected, tid, j, Max_steps_NR = 97; //i and j were moved from unsigned long int to int, Max_steps_NR added added for M33.
    double prob, *radius, k_poly, d, rc, l, x, y, r_outer_limit, Disk_length_by_alpha_IB, Third_alpha_IB_sq, u_max; //IB added l, x, y, r_outer_limit, Disk_length_by_alpha_IB, Third_alpha_IB_sq and u_max.
    double theta, phi, randval, smooth_factor, Disk_height, Disk_length, Disk_length_inv; //IB added Disk_height, Disk_length and its _inv added.
    ...
    double u, Exp_minus_u, Gradient, Error, Tolerance_NR = 1.0e-9; //Added for M33.
    ...
    printf("mcmc_metropolis_hasting_ntry -> IT WORKS!!!!!\n");
    ...
    Disk_height = gal->comp_scale_length[component]*gal->comp_flatz_cut[component];
    Disk_length = gal->comp_scale_length[component];
    Disk_length_inv = 1.0/Disk_length;
    //alpha_IB = 3.0;
    Disk_length_by_alpha_IB = Disk_length/alpha_IB;
    Third_alpha_IB_sq = 1.0/3.0*alpha_IB*alpha_IB;
    u_max = log(cosh(alpha_IB*r_outer_limit*Disk_length_inv));

    hx = gal->comp_mcmc_step[component]*gal->comp_scale_length[component]*gal->comp_flatx_cut[component]; //Extra factor of 6, see if it goes further
    hy = gal->comp_mcmc_step[component]*gal->comp_scale_length[component]*gal->comp_flaty_cut[component]; //Idea is that the step should be about 2 scale lengths, here it works as input file has mcmc_step = 0.3.
    ...
    if(gal->comp_npart[component]>1) {
        ...
        puts("IB altered azimuthal angle of first particle.");
        gal->theta_cyl[i] = atan2(gal->y[i],gal->x[i]); //+pi
        if(gal->theta_cyl[i] < 0.0){
            gal->theta_cyl[i] = gal->theta_cyl[i] + 2.0*pi;
        }
	...
	if(gal->comp_npart[component]==1) return;
        ...
        for(i = gal->comp_start_part[component]+1; i < gal->comp_start_part[component]+gal->comp_npart_pot[component]; ++i) {
			...
			pi_y[k] = density_functions_pool_roy(gal,prop_r[k],prop_theta[k],prop_z[k],1,density_model,component);                    
			//pi_y[k] = dv_y[k]*density_functions_pool_roy(gal,prop_r[k],prop_theta[k],prop_z[k],1,density_model,component);
                }
                weights[k] = pi_y[k];
            }
            // Normalize weights
	    ...
	    //BLOCK COMMENTED OUT TO DISABLE MCMC.
            //randval = gsl_rng_uniform_pos(r[0]);
            //for(k = 0; k<gal->mcmc_ntry; k++) {
			...
			rho_0 = pseudo_density_gas_func(gal,prop_r[k],prop_theta[k],prop_z[k],0,density_model,component,0);
                ...
                if(gal->pseudo[0]) {
                    ...
                    pi_x[k] = density_functions_pool_roy(gal,ref_r[k],ref_theta[k],ref_z[k],1,density_model,component);
		    //pi_x[k] = dv_x[k]*density_functions_pool_roy(gal,ref_r[k],ref_theta[k],ref_z[k],1,density_model,component);
                }
            ...
            //LINE BELOW REPLACED WITH if(1) TO DISABLE MCMC.
            //if(randval <= prob) {
            if(1) {
                ...
                //gal->w[i] = gal->comp_mass_frac[component]/gal->comp_npart[component]*l*exp(-l); //gal->comp_npart[i]
            } else { // Proposal rejected, the particle keeps the same postion
...
void fix_masses(galaxy *gal, int component) {
    double weightTOT, u;
    int i, checker;
    ...
    printf("[   weightTOT = %10.5lf.   ]\n",weightTOT);
    printf("[   component = %d.   ]\n",component);
    printf("[   total mass = %10.5lf.   ]\n",gal->comp_mass_frac[component]*gal->total_mass);
    u = gal->comp_mass_frac[component]*gal->total_mass/weightTOT;

    for(i = gal->comp_start_part[component]+1; i < gal->comp_start_part[component]+gal->comp_npart_pot[component]; ++i) {
	gal->mass[i] = gal->w[i]*u;
	}
    return;
}
...
void mcmc_metropolis_hasting_ntry_mass(galaxy *gal, int component, int imf_model) {
    ...
    printf("mcmc_metropolis_hasting_ntry_mass -> NO!!!!!\n");
...
void mcmc_metropolis_hasting_ntry_stream(stream *st, int component, int density_model) {
    ...
    printf("mcmc_metropolis_hasting_ntry_stream -> NO!!!!!\n");
    ...
    if(st->comp_npart[component]>1) {
        ...
        puts("IB altered azimuthal angle of first stream particle.");
        st->theta_cyl[i] = atan2(st->y[i],st->x[i]); //+pi
        if(st->theta_cyl[i] < 0.0){
            st->theta_cyl[i] = st->theta_cyl[i] + 2.0*pi;
        }
	...
	if(st->comp_npart[component]==1) return;
	    ...
	    if(st->rho[i]<st->comp_dens_min[component]) st->comp_dens_min[component] = st->rho[i];
            ...
            // Updating the coordinate values
            puts("IB altered azimuthal angle of stream particles.");
            st->theta_cyl[i] = atan2(st->y[i],st->x[i]); //+pi
            if(st->theta_cyl[i] < 0.0){
                st->theta_cyl[i] = st->theta_cyl[i] + 2.0*pi;
            }
...
double surface_density_func(galaxy *gal, double r, double theta, int cut, int component) {
    ...
    //surface_density = surface_density/mond_nu_function(2.0*pi*G*surface_density);
    //puts("Hello, evaluating surface density.");

    return surface_density;
}

// Surface density function, including all flattish components. Added by IB.
double surface_density_func_IB(galaxy *gal, double r) {
    double Sigma, u;
    int i;
    
    Sigma = 0.0;
    for (i=0;i<AllVars.MaxCompNumber;i++) {
        if(gal->comp_npart[i]==0) break;
        if(gal->comp_flatz[i] > 0.5) continue;
        u = 1.0/gal->comp_scale_length[i];
        Sigma += gal->total_mass*gal->comp_mass_frac[i]*u*u/(2.0*pi)*exp(-r*u);
    }
    return Sigma;    
}

// gNz function, including all flattish components. Added by IB.
double enclosed_gNz_func(galaxy *gal, double r, double z, double Default) {
    double u, u_1 = 0.0, u_2 = 0.0, rdinv, gNz = 0.0;
    int i;
    ...
    for (i=0;i<AllVars.MaxCompNumber;i++) {
        if(gal->comp_npart[i]==0) break;
        if(gal->comp_flatz[i] > 0.5) continue;
        rdinv = 1.0/gal->comp_scale_length[i];
        u = gal->comp_mass_frac[i]*rdinv*rdinv*exp(-r*rdinv);
        u_1 += u;
        u_2 += u*tanh(fabs(z)*rdinv/gal->comp_flatz_cut[i]);
    }
    if(u_2/u_1 > tanh_2){
        gNz = Default;
    } else {
        gNz = G*gal->total_mass*u_1*tanh_2;
        if(fabs(Default) > gNz){
            gNz = Default;
        } else {
            gNz = gNz*((z > 0) - (z < 0));
...
// Otherwise, the user chose herself the value for the disk scale length.
