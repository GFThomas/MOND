subroutine star2list(name_file,part_out,time_return)
  !-------------------------------------------------------------------------------------------
  !------ DUMPS ALL PARTICLES ----------------------------------------------------------------
  !-------------------------------------------------------------------------------------------
  !----- hacked by G. THOMAS to remove metallicities read for runs with out chemical enrichment
  !-------------------------------------------------------------------------------------------
  implicit none
  integer::ncpu,ndim,npart,ngrid,n,i,j,k,icpu,ipos,n_frw
  ...
  real(KIND=8)::aexp,omega_m,omega_l,omega_b,omega_k,h0,unit_l,unit_t,unit_d,boxlen
  ...
  !================================================
  integer::indice_part_out
  real(KIND=8),intent(out)::time_return
  real(KIND=8),dimension(1:10000000,1:8),intent(out)::part_out
  !================================================
...
!  write(*,*)'time=',(t*unit_t/31557600.d+6)
  time_return=(t*unit_t/31557600.d+6)
  if(TRIM(ordering).eq.'hilbert')then
...
!		print *,m
   ...
   !  write(*,*)'id read'
   !  read(1) ! Skip level
   !  write(*,*)'level read and dropped'
   !  read(1)age
   !  write(*,*)'ages read'
  ! UNCOMMENT to read metallicities if chemical enrichement is on   
  !  read(1)zsol
  !   write(*,*)'Z read'
     close(1)
     ...
     do i=1,npart2
      ...
      !  ok_part=ok_part.and.(age(i).eq.0d0.and.id(i).gt.0)
      !  if(ok_part)then
  ...
  !         write(12,'(I8,1X,10(1PE12.5,1X))')id(i),m(i)*unit_d*(unit_l**3)/2d33,x(i,1),x(i,2),x(i,3),time*1000.,zsol(i)/0.02
            part_out(indice_part_out,1)= (x(i,1)-0.5*boxlen)
            part_out(indice_part_out,2)= (x(i,2)-0.5*boxlen)
            part_out(indice_part_out,3)= (x(i,3)-0.5*boxlen)
            part_out(indice_part_out,4)= (v(i,1)/481.697)
            part_out(indice_part_out,5)= (v(i,2)/481.697)
            part_out(indice_part_out,6)= (v(i,3)/481.697)
            part_out(indice_part_out,7)= m(i)
				part_out(indice_part_out,8)= id(i)	
            indice_part_out=indice_part_out+1	
       !    write(12,'(10(1PE12.5,1X))')(x(i,1)-boxlen),(x(i,2)-boxlen),(x(i,3)-boxlen),(v(i,1)/481.697),&
!(v(i,2)/481.697),(v(i,3)/481.697),m(i)
  !      end if
     end do
     deallocate(x,v,m)
     deallocate(id)
