subroutine grav2ascii(name_file,log_mond,grav_out)
  !--------------------------------------------------------------------------
  ! Ce programme lis les fichier grav et amr en sortie de Ramses pour 
  ! les transformer en fichier ascii par G.THOMAS
  !--------------------------------------------------------------------------
  implicit none
  ...
  integer::ncpu2,npart2,ndim2,nlevelmax2,nstep_coarse2,nboundary2
  real(kind=8)::boxlength
  ...
  real(KIND=8),dimension(:,:),allocatable::potential,potential_N
  real(KIND=8),dimension(:,:,:),allocatable::force,force_N
  ...
  real(KIND=8)::unit_l,unit_t,unit_d,trans_acce,trans_pot
  ...
  integer::id_grav_out
  real(KIND=8),dimension(1:50000000,1:13),intent(out)::grav_out
  !=============================================================
  type level
     integer::ilevel
     integer::ngrid
  ...
  id_grav_out=1
  if(log_mond.EQ.1) MOND=.true.
  if(log_mond.EQ.0) MOND=.false.

  !-----------------------------------------------
  ! Lecture du fichier grav au format RAMSES
  !-----------------------------------------------
  repository=TRIM(ADJUSTL(name_file))
  ...
  read(10,'(A13,E23.15)')GMGM,boxlength
  ...
  close(10)
  !print *,unit_l,unit_t
  trans_pot= (unit_l/100.0)**2.0/unit_t**2.0 ! /100.0 cm->m
  trans_acce= (unit_l/100.0)/unit_t**2.0
  !-----------------------
  ! Map parameters
  !-----------------------
  if(lmax==0)then
  ...
  do k=1,ncpu_read
    ...
    ! ROM: comment the single follwing line for old stuff
     ...
     if(ngrida>0)then
           ...
           allocate(potential(1:ngrida,1:twotondim))
           allocate(force(1:ngrida,1:twotondim,1:ndim2))
           allocate(potential_N(1:ngrida,1:twotondim))
           allocate(force_N(1:ngrida,1:twotondim,1:ndim2))
        endif
           ...
           if(ngridfile(j,ilevel)>0)then
             ...
             if (MOND) then
                 ...
                 do ind=1,twotondim
                   ! Read potential
                    if(j.eq.icpu)then
                      !read(11)potential(:,ind,2)
                      read(11)potential(:,ind)
                   ...
                   ! Read Force
                    do ivar=1,ndim2
                      if(j.eq.icpu)then
                        !read(11)force(:,ind,ivar,2)
                        read(11)force(:,ind,ivar)
                      else
                        read(11)
                      endif  
                    enddo
           ...
           do ind=1,twotondim
              ...
              do i=1,ngrida
                 ok_cell= .not.ref(i).and. &
                      ...
                      & (x(i,2)-dx2)<=ymax.and.&
                      & (x(i,3)-dx2)<=zmax
                 if(ok_cell .AND. ((potential(i,ind) .NE. 0.00) .OR. &
&(force(i,ind,1) .NE. 0.0) .OR. (force(i,ind,2) .NE. 0.0)&
& .OR. (force(i,ind,3) .NE. 0.0))) then
!print *,i,id_grav_out
                   grav_out(id_grav_out,1)=((x(i,1)-0.5)*boxlength)
                   grav_out(id_grav_out,2)=((x(i,2)-0.5)*boxlength)
                   grav_out(id_grav_out,3)=((x(i,3)-0.5)*boxlength)
                   grav_out(id_grav_out,4)=(dx*boxlength)
                   grav_out(id_grav_out,5)=(potential(i,ind)*trans_pot)
                   grav_out(id_grav_out,6)=(force(i,ind,1)*trans_acce)
                   grav_out(id_grav_out,7)=(force(i,ind,2)*trans_acce)
                   grav_out(id_grav_out,8)=(force(i,ind,3)*trans_acce)
                   grav_out(id_grav_out,9)=ilevel
						 if (MOND) then
	                   grav_out(id_grav_out,10)=(potential_N(i,ind)*trans_pot)
	                   grav_out(id_grav_out,11)=(force_N(i,ind,1)*trans_acce)
	                   grav_out(id_grav_out,12)=(force_N(i,ind,2)*trans_acce)
	                   grav_out(id_grav_out,13)=(force_N(i,ind,3)*trans_acce)
						 else
	                   grav_out(id_grav_out,10)=(potential(i,ind)*trans_pot)
	                   grav_out(id_grav_out,11)=(force(i,ind,1)*trans_acce)
	                   grav_out(id_grav_out,12)=(force(i,ind,2)*trans_acce)
	                   grav_out(id_grav_out,13)=(force(i,ind,3)*trans_acce)
						 end if			
                   id_grav_out=id_grav_out+1
                 end if
              end do
           ...
           deallocate(xg,son,potential,force,potential_N,force_N,ref,x)
        endif
...
contains
  ...
  subroutine read_params
    ...
    n = iargc()
    if (n < 4) then
       print *, 'usage: grav2ascii  -inp  input_dir'
       print *, '                 -out  output_file'
       print *, 'ex: grav2ascii -inp output_00001 -out column.dat'
       print *, ' '
       stop
    end if
