...
// MOND interp function -> Graeme's addition
double mond_nu_function(double f) {
    double mond_nu; //% Variables n, y deleted by IB.
    //%mond_a0=1.2*pow(10.0,-13);
    //n = 1.0;
    if(mond_forces == 1) {
        //y = f/mond_a0;
        // mond_nu = pow(0.5*(1.0 + sqrt(1.0 + 4.0/pow(y,n))),1.0/n);
        mond_nu = 0.5 + sqrt(0.25 + mond_a0/f);
    } else {
        mond_nu = 1.0;
    }
    return mond_nu;
...
//%K_0 function added by IB.
double mond_K_0_function(double f) {
    double y_inv, nu, K_0;
    if(mond_forces == 1) {
      y_inv = mond_a0/f;
      nu = 0.5 + sqrt(0.25 + y_inv);
      K_0 = -0.5*y_inv/(nu*(nu - 0.5));
    } else {
      K_0 = 0.0;
    }
    return K_0;
...
static double dv2a_r_2D_func(double z, void *params) {
    ...
    radius = fabs(gal->r_cyl[gal->index[tid]]);
    ...
    integrand = rho*galaxy_zforce_func(gal,radius,z);
...
void fill_jeans_3D_grid (galaxy *gal, int component) {
    int i, j, k, n, nsteps, tid, radius; //added radius
    ...
    radius = fabs(gal->r_cyl[gal->index[tid]]); //added this for use in force_func
        ...
        if(k==gal->ngrid_jeans[0]-1) {
            ...
            for (n=0; n<nsteps; n++) {
                ...
                for (j=0; j<gal->ngrid_jeans[1]; j++) {
                    ...
                    p = rho*galaxy_zforce_func(gal,r1,z);
    ...
    for (k=gal->ngrid_jeans[0]-1; k>=0; k--) {
        for (j=0; j<gal->ngrid_jeans[1]; j++) {
            ...
            double fz = galaxy_rforce_func(gal,r1,z1);
...
double v2a_r_toomre(galaxy *gal, double radius, double v2a_r, int component) {
    ...
    double abserr, x, y, v2a_r_new, g_N_r, g_N_z, g_N_total, Half_K_0, G_Sigma_eff; //Variables after v2a_r_new added by IB.
    ...
    surface_density = surface_density_func_IB(gal, radius); //surface_density_func(gal,radius,gal->theta_cyl[gal->index[tid]],1,component);
    g_N_r = galaxy_rforce_func_Newton(gal,radius,gal->z[gal->index[tid]]);
    g_N_z = 2.0*pi*G*surface_density;
    g_N_total = sqrt(g_N_r*g_N_r + g_N_z*g_N_z);
    Half_K_0 = 0.5*mond_K_0_function(g_N_total);
    G_Sigma_eff = G*surface_density*mond_nu_function(g_N_total)*(1 + Half_K_0);
    if(gal->comp_type[component]==0) {
        Q = sqrt(gal->comp_cs_init[component]*fabs(kappa_sqrd))/(pi*G_Sigma_eff); //G*surface_density replaced with G_Sigma_eff.
    } else {
        Q = sqrt(v2a_r*fabs(kappa_sqrd))/(3.36*G_Sigma_eff); //G*surface_density replaced with G_Sigma_eff.
    }
    // Check the value of the Toomre parameter
    if(gal->comp_Q_boost[component]>0.) {
        Q = Q + gal->comp_Q_boost[component];
        v2a_r_new = pow(Q*3.36*G_Sigma_eff,2.0)/(fabs(kappa_sqrd)); //G*surface_density replaced with G_Sigma_eff.
    }
    if(Q < gal->comp_Q_lim[component] && gal->comp_Q_lim[component]>0) {
        v2a_r_new = pow(gal->comp_Q_lim[component]*3.36*G_Sigma_eff,2.0)/(fabs(kappa_sqrd)); //G*surface_density replaced with G_Sigma_eff.
        Q = gal->comp_Q_lim[component];
    }
    if(gal->comp_Q_fixed[component]>0) {
        v2a_r_new = pow(gal->comp_Q_fixed[component]*3.36*G_Sigma_eff,2.0)/(fabs(kappa_sqrd)); //G*surface_density replaced with G_Sigma_eff.
...
double toomre(galaxy *gal, double radius, double v2a_r, int component) {
    ...
    double x, y, save, g_N_r, g_N_z, g_N_total, Half_K_0, G_Sigma_eff; //Variables after save added by IB.
    int tid;
...
double v_c_func(galaxy *gal, double radius) {
    ...
    //fabs(gal->r_cyl[gal->index[tid]]); //%Blocked by IB.
    z = gal->z[gal->index[tid]]; //%fabs removed by IB, should use actual z.
    rforce = galaxy_rforce_func(gal,radius,z);
    //v_c = sqrt(radius*rforce); //%IB only calculates v_c if rforce > 0.0.
    //if(rforce<0) return 0.;
    //else return v_c; //%IB replaced these two lines with safer block below.
    if(rforce <= 0.0){
        return 0.0;
    } else {
...
// These functions calculate the force on a test particle due to the disk
// at a point with cylindrical radius r. This is used to calculate the velocity dispersion.
//
// These functions simply perform a five point differentiation around
// x,y,z in the cylindrical radius direction with a small stepsize.
//
//These functions are specially formatted for the velocity dispersion
// routines.

//Radial force in Newtonian gravity.
double galaxy_rforce_func_Newton(galaxy *gal, double radius, double z) {
    int tid;
    double g_N_r, h, abserr;
    double x,y,r_sph;
    ...
    y = radius*sin(gal->theta_cyl[gal->index[tid]]);
    h = get_h_value(gal,x,y,gal->z[gal->index[tid]],0,0);
    g_N_r = deriv_central2(gal,radius,h,galaxyr_potential_wrapper_func);
    return g_N_r;
}


double galaxy_rforce_func(galaxy *gal, double radius, double z) {
 
    int tid;//,mond_forces;
    double force, h, abserr, g_N_z, z_threshold; //added g_N_z and z_threshold.
    double x,y,r_sph;
    double sigma,transition_factor1,transition_factor2;
    ...
    // MOND (spherically symmetric approx) -> Graeme's Addition
    if (mond_forces == 1) {
      //z_threshold = 2.0*gal->comp_scale_length[0]*gal->comp_flatz_cut[0];
      //  if ((z>z_threshold) || (z<-z_threshold)) {
      //if (fabs(z) > gal->zthreshold) {
      //     g_N_z = deriv_central2(gal,z,h,galaxyz_potential_wrapper_func);
      //  } else if (z >= 0) {
      //      g_N_z = 2.0*pi*G*surface_density_func_IB(gal, radius)*tanh(2.0);
      //  } else {
      //      g_N_z = -2.0*pi*G*surface_density_func_IB(gal, radius)*tanh(2.0);
      //  }
      g_N_z = enclosed_gNz_func(gal, radius, z, deriv_central2(gal,z,h,galaxyz_potential_wrapper_func));
      force = mond_nu_function(sqrt(force*force + g_N_z*g_N_z))*force;
    }
    return force;
}
 

// This function calculates the force on a test particle due to the disk at a point with an height z. This is used to calculate the velocity dispersion.
// This function simply performs a five point differentiation around x,y,z in the z direction with a small stepsize.
// This function is specially formatted for the velocity dispersion routines.
double galaxy_zforce_func(galaxy *gal, double radius, double z) { //added extra input to
 
    int tid;
    double force, g_N_z, h, abserr, g_N_r, z_threshold; //added u, g_N_r and z_threshold.
    ...
    if(mond_forces == 1) {
      ...
      //      g_N_z = -2.0*pi*G*surface_density_func_IB(gal, radius)*tanh(2.0);
      //  }
      g_N_z = enclosed_gNz_func(gal, radius, z, force);
      g_N_r = deriv_central2(gal,radius,h,galaxyr_potential_wrapper_func);
      force = mond_nu_function(sqrt(g_N_z*g_N_z + g_N_r*g_N_r))*force;
    } 
    return force;
}


// This function calculates the force on a test particle due to the global potential at a spherical radius r_psh. This is used to calculate the velocity dispersion.
//
// This function simply performs a five point differentiation around x,y,z in the spherical radius direction with a small stepsize.
//
// This function is specially formatted for the velocity dispersion routines.
double galaxy_rsphforce_func(galaxy *gal, double r_sph) {
    ...
    // MOND (spherically symmetric approx) -> Graeme's Addition
    if(mond_forces == 1) force = mond_nu_function(force)*force;
    return force;
}
