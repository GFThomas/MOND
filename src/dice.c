...
int main (int argc, char **argv) {
    ...
    int i, j, k, l, ngal, N, neval, p; //p Added by IB.
    double x,y;
    // Galaxy pointers
    ...
    puts("MOND patch by Roy Truelove, assisted by Indranil Banik, Graeme Candlish and Ingo Thies.");
    printf("/////\n");
    printf("/////\t--------------------------------------------------\n");
    ...
    for(k = 0; k<AllVars.Ngal; k++) {
	    ...
	    puts("Fixing particle masses. 1"); //Added by IB.
           for(p = 0; p<AllVars.MaxCompNumber; p++) {
         if(stack->comp_npart[p]>0) {
 	     fix_masses(stack,p);
         }
      }
            // Next iterations
		...
		puts("Fixing particle masses. 2"); //Added by IB.
           for(p = 0; p<AllVars.MaxCompNumber; p++) {
         if(gal->comp_npart[p]>0) {
 	     fix_masses(gal,p);
         }
      }
	    ...
	    puts("Fixing particle masses. 3"); //Added by IB.
           for(p = 0; p<AllVars.MaxCompNumber; p++) {
         if(stack->comp_npart[p]>0) {
 	     fix_masses(stack,p);
         }
	...
	puts("Fixing particle masses. 4"); //Added by IB.
           for(p = 0; p<AllVars.MaxCompNumber; p++) {
         if(pgal->comp_npart[p]>0) {
 	     fix_masses(pgal,p);
         }
      }
    ...
    if(strcmp(AllVars.ICformat,"Gadget1")==0) { // 0 means the strings match.
        printf("/////\tWriting <%s.g1> file [%s format]\n",AllVars.Filename,AllVars.ICformat);
	    //puts("Writing of Gadget1 ICs disabled by IB."); //Added by IB.
        write_gadget1_ics(stack,AllVars.Filename);
    }
    if(strcmp(AllVars.ICformat,"Gadget2")==0) { // 0 means the strings match.
        printf("/////\tWriting <%s.g2> file [%s format]\n",AllVars.Filename,AllVars.ICformat);
	    //puts("Writing of Gadget2 ICs disabled by IB."); //Added by IB.
    ...
    printf("/////\tDisk template successfully created in %d seconds.\n",(int)ceil(cpu_time));
    printf("/////\t--------------------------------------------------\n");
    return 0;
}
